...
* Advanced settings
  * manage/add application metadata tab
    ...
    * max 255 characters
    * can have up to 10 key/value pairs of metadata
  * device settings tab
  ...
  * OAuth tab
    ...
    * Compliance enforcement level
      * default: none
    * Non-Verifiable Callback URI End-User Confirmation - user has to login when callback is non verifiable
      * default: on
...
* Universal login handles invalidating any active refresh tokens when call /v2/logout
* New one issued when current refresh token is used. Old one is invalidated
  ...
  * If an older refresh token is used then the entire fresh token family (all refresh tokens issued for a user) are 
    invalidated and user is forced to re-authenticate
* Recommends
  * automatic reuse detection
  * refresh token rotation (on by default public apps but not confidential)
* SDK: supports refresh token management
* Affects flows
  * Authorization Code Flow (with/without PKCE)
  * Device Authorization Flow - not supported by Enterprise connections using OIDC
  * Resource Owner Password Flow
* Lifetime settings
  * NOTE invalid_grant error indicates either max refresh token age hit (resulting in re-authentication) or 
    refresh token reuse (potential theft) 
  * NOTE: strangely this is not the case for SPA, their timeouts are much shorter - idle = 15 days, max = 30 days
  * Idle
  ...
  * max
    ...
    * max: 1 year (31,557,600 seconds or 365.25 days)
* Limitations
  * refresh tokens are bound to a specific client id and audience they were issued for 
...
* (New) OIDC Logout - RP-initiated endpoint (Relaying party - service or application, /oidc/logout or /oidc/logout?federated for 3rd party IdPs also)
  ...
  * Only signs you out of the current application immediately
    ...
    * If you need to be signed out of all applications immediately
      * OIDC Back Channel Logout (/event/backchannel-logout) - (MORE SECURE). See below for details. All done outside of browser
        NOTE: not used to log out of identity providers - use /v2/logout?federated for 3rd party IdPs 
      ...
      * SAML Single Logout (SLO) - Application OR IdP initiated
        * NOTE: not to be confused with when enterprise SAML connection being used. You can just use ?federated and 
        you'll be logged out
        * NOTE: what follows is when Auth0 is the SAML provider i.e. you just want Auth0 to provide a SAML interface 
        not federate with an existing SAML IdP
        1. Settings: application > addons > SAML ADDON toggle > settings
        2. enable 'callback' - enter the SLO URL of the SAML application e.g.
        3. set SLO to enabled
        * e.g. 
        ```json
        {
         "logout": {
  ...
  * Redirect
    * Must be added to Allowed Logout URLs
      * Application settings: If you provide the id_token_hint parameter, or the logout_hint and client_id parameters in your call
        (you provide these in you actual app call) otherwise will fallback to Tenant settings
      * Tenant settings: If you provide only the logout_hint parameter in your call
* Redirect URLs (Common to both)
  ...
  * In Allowed callback URLs field - returnTo (old) or post_logout_redirect_uri (new) query parameter must be un-encoded 
    (comma separated, * (wildcards) for subdomains, and case-sensitive). Query parameter validation is ignored for returnTo
  ...
  * Allowed Logout and Allowed Web origins are limited to 100 URLs
  * Organisations - Alternatives to wildcards
    * For multi-tenant B2B applications that use the Auth0 Organisations feature, a more secure and dynamic method for 
    handling customer-specific subdomains is available. Instead of using a risky wildcard, the {organization_name} 
    placeholder can be used in the Allowed Callback URLs, for instance: https://{organization_name}.myapp.com/callback
    NOTE: wildcards are not supported when using {organization_name} placeholder.
...
* Flow
  1. Login - During user authentication with apps, the Auth0 tenant provides **ID token** with sid and uid (session id and user id)
...
* Limitations
  ...
  * Any changes to endpoint address must be synced with Auth0 tenant

## Configure cross-origin authentication
* Cross-origin authentication is not recommended and is only necessary when authenticating against a directory using a username and password
* Configured as Application > Allowed origins (CORS)
* Workaround: When 3rd party cookies are blocked, SDK > corssOriginVerification set to dedicated page with non cross origin domain
...
* Key differences between Universal Login and Embedded Login
  * Hosted - You are responsible for handling redirects and token storage correctly
...
* Lock for Web / Custom UI
  ...
  * Custom signup
    * Lock sends custom fields to /dbconnections/signup (behind scenes). Stored in user_metadata
    * User metadata
...
* Identifier + Biometrics - when possible users have ability to choose WebAuthN method instead of password
* Enable home realm discovery
  * Not available for Identifier + password
  * Only works with Identifier first so it can redirect to correct IdP when email provided
  * When email provided user is forwarded to IdP for authentication
  * Configured in Dashboard > Authentication > Enterprise
  * Can configure up to 1000 domains associated with domain for HRD purposes
* MFA
...
* Device support
  * user must have another MFA enrollment activated before using device biometrics
  * WebAuthN with biometrics - can never be used as second factor. Always progressive enrollment prompt
...
NOTE: You must keep token sizes small to avoid request header and cookie size limits. Cookies are limited to 4KB. 
...
* Verify standard claims
  ...
  * Authorised Party (azp) - must match clientID
...
* Defaults
  * token expiration - 36,000 seconds (10 hours)

### Access Tokens
* aud - set to API identifier
* type - used to authorise
  * Opaque - used to get scopes from Auth0 (good if you have lots of scopes, bad because extra API call). If not 
    audience is set then this is the default returned access token
...
* Normalise a profile - just Auth0 mapping OIDC user profile to a common profile in Auth0 for that user for consistent exchanges with Auth0
* B2C
...
* B2B - closed enterprise connections whereby you can invite business partners to join your tenant (organisations)
  * identity lifecycle is managed by the external enterprise IdP - registration, email verification, etc...
  * you must provide developer keys for all environments
...
* 4 types
  * Database (Password) (B2C)
    ...
    * Naming: alphanumeric characters plus dashes with max length 35 characters
...
* LDAP requires special treatment
  * Auth0 AD/LDAP Connector
    ...
    * must use config.json file to configure properties not available via extension admin console
...
* Support
  * management API - /api/v2/jobs/users-imports - (Recommended, more reliable, more flexible because more functionality)
  * Auth0 Dashboard
* Other options
  * upsert - if user exists, update user record
  * send_completion_email - controls whether Auth0 sends a notification email when job is finished
* Recommendations
  * Split large migrations into multiple files
  * Start with just 1-5 users at first
