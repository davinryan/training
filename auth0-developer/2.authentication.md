...
* Redirect URLs (Common to both)
  ...
  * In Allowed callback URLs field - returnTo (old) or post_logout_redirect_uri (new) query parameter must be un-encoded (comma separated, * (wildcards) for subdomains, and case-sensitive). Query parameter validation is ignored
...
* Enable home realm discovery
  ...
  * Only works with Identifier first so it can redirct to correct IdP when email provided
  * When email provided user is forwarded to IdP for authentication
  * Configured in Dashboard > Authentication > Enterprise
  * Can convfigure up to 1000 domains associated with domain for HRD purposes
* MFA
