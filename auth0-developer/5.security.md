...
## Auth0 Support for mTLS & DPoP
Overview: When required, access tokens must be constrained to a specific application and client (caller) to prevent 
unauthorized use of leaked or stolen tokens.
* Options
  * mTLS - Mutual TLS - requires client certificate to be presented during authentication (Confidential clients only)
  * DPoP - Delegated Proof of Possession - requires proof of possession of a private key to be presented during authentication
    * DPoP is a more secure alternative to mTLS as it does not require client certificates
* Cons - lots of overhead and requires client certificates to be generated and distributed. Normally we just use one 
...
* Reset users MFA
  ...
  * Management API - delete MFA enrollments
    * DELETE `api/v2/users/{userId}/authentication-methods/{authentication_method_id}` 
      (scopes: delete:authentication_methods or update:users) endpoint removes **ONE** of the enrollments
    * DELETE `api/v2/users/{userId}/authenticators` removes **ALL** authenticator apps such as the Auth0 Guardian app 
...
* Limitations
  ...
  * Score based on
    ...
    * UntrustedIP - IP that's on a bad behaviour list
...
* 'Step up authentication' via scopes with Actions
  * NOTE: post-login actions cannot be used silently. If using silent login then you must check amr claim for presence
    of mfa and send user through new authorisation request using acr_values=http://schemas.openid.net/pape/policies/2007/06/multi-factor
    to signal to Auth0 that MFA is required. You still need to provide a post-login action to check for MFA e.g.
    ```javascript
      exports.onExecutePostLogin = async (event, api) => {
  ...
  * 'Customise MFA factors using actions' ensures verification logic will be available using extensible methods in actions
  ...
  3. Check if ID token 'amr' claim equals 'mfa' (already passed MFA)
...
* Other configuration
  * count_per_identifier - number of failed attempts per identifier (default: 10)
  * count_per_identifier_and_ip - number of failed attempts per identifier and IP address (default: 10)
...
* As usual you'll be in monitoring mode if you do not enable this feature
* 3 ways
  * Block new users from signing up with compromised credentials
...
* Configuration
  * Detection method
    * Credential Guard - uses dark web, researches, more active methods to detect breached passwords (unknown lag)
  ...
  * Notifications
    * Send and email each user of the above responses
...
* If you do not provide a response, then Bot Detection will just monitor. Good for trialing/testing
* Responses
...
the **nonce claim** is the defence. Similar to state, the application generates a unique value for nonce and sends it 
