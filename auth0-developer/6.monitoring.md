...
* Log Streaming and exporting - ability to send to a tool of your choice for analysis
...
* Management API Location
  ...
  * Search by Checkpoint
    ...
    * Limitations (searching by checkpoint)
      ...
      * forward only
      * each checkpoint id is valid for 24 hours
      * you'll get a checkpoint id for calling the next checkpoint
  * Search logs by criteria
    * Query params
      * q - search query - uses Lucene query format
...
### Common commands you MUST be across
...
| Tickets                   | /tickets/email-verification<br/>/tickets/password-change | POST               | Generate verification or password-reset URLs.     |
...
* Steps
  1. Install - `npm install -g auth0-deploy-cli` or use Terrform Auth0 Provider
...
* Typical way to use Auth0 Management API
  * Limitations
    * Cannot log users in on their behalf
    * Enforces strict rate limits which may impact functionality in production (they talk about this A LOT. Suspect its 
      due to many service requests and issues raised)
  * Test environments
    1. Go to tenant Auth0 Management API
    2. Select API Explorer tab
    3. Click 'Create & Authorize Test Application'
    4. Click 'Copy icon' to copy the token
    5. Navigate to Auth0 Management API via Documentation menu
    6. Click 'Set API Token', copy token and click 'Set Token'
  * Production
    * Use Client Credentials Grant to get token instead
    * Only use scopes required to perform desired operation
    * You can use node-auth0 SDK package to call Management API
...
* Install
  * Windows
    ...
    scoop install auth0
  * Linux (get link from https://github.com/auth0/auth0-cli/releases)
...
* Raw Management API call
  * `auth0 api --data '' --scopes`

### When to use what (Exam Cues)
* Quick task from terminal - use Auth0 CLI
* Repeatable environment sync - use config as code, CI/CD - Deploy CLI
* Both tools call the Management API under the hood 
  (why they do not combine these I suppose it's a security concern)