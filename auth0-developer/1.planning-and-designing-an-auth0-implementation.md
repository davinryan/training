...
* Key features that are not supported or unique to classic login
  * Email magic link - because OTP code not supported on UI
  ...
  * Ability to disable self-service password recovery
  * Custom URLs for password reset and user signup
  * Kerberos support for AD/LDAP connections
  * Requires exposing identify provider domains in a public endpoint
