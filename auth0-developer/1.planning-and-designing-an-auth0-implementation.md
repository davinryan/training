...
Exam questions in this domain test the understanding of Auth0’s architectural building blocks and the security 
implications of fundamental configuration choices
...
### Classic Login
* NOTE: can only update pages via /api/v2/tenants/settings for HTML & Javascript and not Liquid
  * login
  * password reset
  * MFA Authentication
* Not fully deprecated, considered legacy
* Key features that are not supported or unique to classic login
  * Passwordless Email magic link - because OTP code not supported on UI
  * Terms of service acceptance on signup
  * Custom fields on signup
  * Javascript is required
  * No new features to be supported
  * No support for
    * Identifier first login 
    * Passkeys
    * WebAuthn, device biometrics
    * Web content accessibility guidelines compliance
    * Organisations - 1000 domains per organisation
    * Customisable localised text on all pages
    * Email MFA
    * Voice MFA
    * MFA customisation with actions
    * Buttons for enterprise connections
  * Kerberos support for AD/LDAP connections (Universal does not support)
  * Requires exposing identify provider domains in a public endpoint (Universal does not support)
...
* Passkeys - also available as an authentication method for database connections (not available for classic)
* MFA - can be customised with Actions and user metadata can be leveraged to create more individualised experiences
  * Duo MFA - supported but must be the only factor
* Sign Up
* No support for Kerberos via AD/LDAP connections
* Password Reset - can be customised with Actions to provide secondary challenge
* Custom databases - if error when changing password, will still be fine even if change password script fails 
(password in Auth0 just not in custom database I suppose)
* Email Verification - only OTP NOT magic links
* Supports password autocomplete with password managers
* Customised experience - Data capture (extra data from user) & Custom content (like branding, logos)
...
* Not displayed: authorisation error and callback URL is valid - error returned to the application via query parameters 
  state and error in callback URL
...
* Template properties
  ...
  * have access to common variables (tenant, user and application meta data only) (e.g. {{ user.name }})
...
* Location to set: 
  * Enterprise: Select an enterprise connection > Login Experience > enter as many email domains as you want to associate with this connection
  * Applications: Select an application > Login Experience > select organisation (assumes you have created an organisation and associated it with email domains)
...
* Auth0 built in feature stuff
  ...
  * Brute Force Protection
    * Server side API problems - Cannot easily be used with Bruteforce protection
      * Uses the host server IP address instead of a users IP address due to the nature of its use
      * Fix: configure auth0 application to trust IP address
        1. Settings > Advanced Settings > OAuth tab > enable 'Trust Token Endpoint IP Header' which will set the 'auth0-forwarded-for'
          header as a trusted source of the user's IP address for brute-force protection
        2. Send the user's IP address from your server in the 'auth0-forwarded-for' header when POSTing to /oauth/token
        3. Add any servers that do not require brute force protection to the IP AllowList in Auth0
        4. Validate in logs
          ```text
          type:  sepft
          ...
  ...
  * Breached password detection
    * Client side issues - need to configure to handle error message returned
      ```json
      {
...
### MFA
* Something you know, have, are - MFA covers at least 2 of these
  * Or Knowledge, possession, inherence 
...
* Options
  * Auth0 Guardian - God help us - you have no control over this, its propriety, sucks to be you if you don't like the app or it doesn't work for you
    * Receives push notification - App can provide more than one factor -  its own biometrics, OTP as additional checks
  ...
  * Voice - God help us - AI can imitate human speech
  ...
  * Phone message
  * FIDO with WebAuthn Device biometrics - Phishing resistant, can provide more than one factor (not really true, but cannot be used on its own so has to use another MFA type)
  * PassKeys - phishing-resistant because they are bound to an 'origin' and a fake domain cannot satisfy the cryptographic origin check
* Authenticator Assurance Level (AAL)
  * AAL1 - Single knowledge factor - low confidence
    * e.g. Password
  * AAL2 - at least Two factors - Knowledge + (Possession OR Inherence)- high confidence
    * e.g. Password + OTP
    * Approved cryptographic techniques 
    * e.g. Regular sites
      * Password login + Guardian app push notification (Possession)
      * Password login + Synced passkey (Possession/Iherance)
  * AAL3 - same as AAL2 but requires 'Hard' cryptographic authenticator
    * e.g WebAuthn, FIDO security key 
    * cryptographic authenticator provides impersonation resistance (resistant to phishing or man-in-the-middle attacks)
    * e.g. Banks, financial institutions
      * Password login + WebAuthn with FIDO security keys (Possession)
      * Password login + single device bound Passkey (Possession/Iherance)
  * Based on 2 risk factors to determine AAL level
    * Potential harm or impact
    * Likelihood of harm or impact
* Auth0 recommends AAL2 for most apps
...
* In order to make this work (if your app initiates invite, not Auth0)
  ...
  2. Specify route behaviour - update app to call login URL with invitation ticket id and organization id 
     e.g. https://myapp.com/login?invitation={invitation_ticket_id}&organization={organization_id}&organization_name={organization_name}
...
* Client credentials flow - used for server to server communication (not OIDC compliant)
* Device authorization flow - for devices without a browser or with limited capability (smart TVs, IoT devices, CLI tools), 
  Mobile apps should use 'Authorisation code with PKCE' flow
* Resource owner password flow - used for CLI apps (basically just username & password). Only use when browser based redirect is not feasible
* Client initiated backchannel authentication (CIBA) flow (used with PAR, RAR, JAR) - used for mobile apps
* Custom token exchange - basically just using the existing flow above and then manually call /oauth/token endpoint to access
...
* Features
  ...
  * B2B (business to business)
    ...
    * Self service signup
    * Any closed enterprise connection (e.g. Organisations) whereby you can invite people (partners)
...
* 10 workstreams required to go live (essentially architecture)
  ...
  8. Profile management - KYC (Know Your Customer) two types of meta stored: user editable; not user editable 
     (recommended: never store meta that impacts access control that users can edit)
...
* Subscription tiers
  * Free - 1 production tenant
  * Paid - more than 1 production tenant (depends on subscription)
...
* Recovery (when dashboard access is lost)
  * (LOL) use an email account not linked to a single user – access old users email and reset from their account (dumb idea)
  * Check for another admin on tenant - (you're fucked)

## Multi-Tenant Application Best Practices
* Auth0 recommends Auth0 Organisations (Groups of Users with tailored experiences)
* Use cases
...
* Opaque tokens - Auth0 validates this when you send opaque token to Auth0 via /oauth/token/introspect endpoint
...
* Refresh token usage
  ...
  * if you try to redirect with context.redirect the auth flow will return an error
  ...
  * Auth0 recommendation: Instead of manually deleting refresh tokens just set **an expiry** time
* JWT Validation
  ...
  * Signing Keys
    ...
    * cache these to improve performance, token validation failure should result in cache invalidation (only one time) 
...
* Legacy Database - Use your own user store
  * Reasons: Auth0 try before you by; forced to keep user store
  * Basically just migrate application platform not data
  * Requires custom database
  * User store must be available from the public internet (Auth0 can't access private systems)
* Options
  * Automatic migration followed by bulk migration when you're ready to say goodbye to legacy IdP
...
* Single Page Applications (SPA)
  * No backend
  * Authorisation Code Flow with PKCE recommended
  * Token storage: local/session storage, in-memory (preferred)
  * Session management: Token based sessions
  * Ensure storage is memory-first
...
* Machine to Machine Applications
  ...
  * Client Credentials Flow recommended - not ID token returned, just access tokens only as no user context
  * Token storage: secure server side
  * Session management: Stateless, uses tokens for authentication

### Tips
* Always have to add an 'API' if you want to use any apps (not any more - only if you need audience to be populated in token)
