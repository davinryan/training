...
* Dashboard or management API
  ...
  * Templates - Branding > Email Templates
  * Management API - /api/v2/emails/provider
* Monitor deliverability, 'from address' and domain settings

### Configure custom domain for authentication
* Must add CNAME record to allow mapping to Auth0
...
* Limitations
  * no access to client_metadata variables 
  * Redirect URL does not have access to Auth0 template variables only application.name and application.callback_domain
...
* Uses Liquid templating language
  ...
  * Key features
    ...
    * Styling - _widget-auto-layout - special class to center prompt on page (only works on body)
  * Limitations
...
### Actions
* IMPORTANT - actions always override tenant level policies
  * e.g. if MFA set to never for tenant, and action requires MFA using api.multifactor.enable then users will be 
    prompted for MFA regardless of tenant policy
* Are added to triggers and executed asynchronously or synchronously
...
* Types
  * Sign up and login triggers
    * post-login - synchronous - blocks authentication until action is complete
      * Use cases: denying access based on custom criteria, adding custom claims to tokens, or redirecting a user for progressive profiling
    * pre-user-registration - synchronous - triggers before a user is created for database or passwordless connection
      * Use cases: validate incoming user data or to deny registration attempt based on business logic; also setting metadata in user profile (even though its not created yet)
    * post-user-registration - asynchronous - triggers asynchronously after a user is created
      * Use cases: sending a welcome email, creating a user record in CRM or other downstream tasks
  * MFA Notification triggers
    * send-phone-message - Triggers before using custom provider to send the messages for the enrollment and the challenge process
  ...
  * Custom token exchange trigger
    ...
    * Usage: token delegation — where a service needs to act on behalf of a user to call a downstream API with a 
      different audience, exchanging the original token for a new scoped one without requiring the user to re-authenticate.
* Main parameters
  * event - object for post-login actions triggers that provides contextual information about a single user logging in via Auth0
    ...
    * Key fields
      * event.user - user profile including user/app metadata
      * event.client - app initiating the transaction
      * event.connection - the identity provider connection used
      * event.organization - details of org, id, name etc...
      * event.request - info about inbound http request such as ip address and geoip data
      * event.secrets - access to secrets stored in action context
      * E.g. test for MFA event.user.multifactor has value OR event.authentication.methods array includes item with name = 'mfa'
  * api - object for actions that allows you to: manipulate tokens and user profiles; deny access to applications;
    ...
    * Key functions
      * api.access.deny(reason, userMessage) - immediately halts flow and denies user access
      * api.idToken.setCustomClaim(name, value) - addes namespaced custom claim to the id token - only available in post-login trigger
        NOTE: URL like namespaces are important to avoid collisions with OIDC claims
      * api.user.setUserMetadata(name, value)/api.user.setAppMetadata(name, value)
      * api.redirect.sendUserTo(url, {query: {...}}) - reidrects the user to an external URL, pausing auth flow. 
        Use https://${event.request.hostname}/continue to continue from remote app 
      * api.multifactor.enable('any', { allowRememberbrowser: true})/api.authentication.challengeWith(..) - Programmatically enforce an MFA challenge, 
        overriding tenant level policies. Only available in post-login trigger
* Testing
  * You can use debugging (via logs) or pre-production testing (preferred answer strangely enough)
* Limitations
  * 20 seconds execution time cap
  * NPM packages only
  * 100KB of code per action
  * Calls made to Management API are rate limited
  * 100 actions per tenant
  * 20 active bound actions per tenant
...
* Rules & Hooks are deprecated so don't use them (Rules and Hooks EOL is November 18, 2026)
  * Order in which these run: Rules first, Hooks second and finally Actions
  * Limitations
    * Rules - limited to authentication pipeline
    * Hooks - limited to pre-user registration, password changes and creating a disjointed experience
    * Action - consolidate the above into one framework with versioning, rollback, secrets and access to npm registry
* Allow you to install applications, scripts/commands that extend the existing functionality of Auth0 base product
* NOTE: Third party add-ons are supported. What follows are Auth0 provided add ons
* Main Extensions
...
* SAML Mapping extension
  * Location: applications > application > addons > Saml2 toggle
  * allows you to map SAML attributes to Auth0 user profile attributes in case of bad data from enterprise SAML IdP
  * mappings defined in json
    ```json
    "mappings": {
...
* NOTE: anything page updates to /api/v2/tenants/settings are classic pages that can only use HTML & Javascript and not Liquid
* Custom error page is just a link to a custom page hosted by you
* Why you provide a custom error page you'll get the following query parameters
  * client_id - if it was provided in the transaction
  * connection
  * lang
  * error - error code
  * error_description - description of the error
  * tracking - id used by Auth0's logs for quick lookup
* Location: dashboard > tenant settings > Error Pages (generic/custom)
* Management API - Redirect to custom error page
  *  show_log_link - whether to show a link to the Auth0 logs
  ```bash
  curl --request PATCH \
  ...
  ```
* Management API - provide your own HTML
  ```bash
  curl --request PATCH \
...
### Design and implement user group management (WTF is this!! - Enterprise feature)
* SCIM - System for Cross-domain Identity Management
* Enterprise connections only
* Basically it's a common interface and schema for Syncing user accounts to Auth0 from an enterprise identity provider
  (very similar to how Auth0 syncs user info on login from enterprise connection - this is better)
* This is probably a good idea when enterprise connection doesn't support userinfo endpoint or not OIDC compliant
  feels hacky to use SCIM, probably legacy, deprecated and out of date
* It's an inversion of control. Normally Auth0 syncs user info. This endpoint allows a remote enterprise identity provider 
  to sync user profiles to Auth0 instead using SCIM schema
* Enable: 
  1. dashboard > Authentication > Enterprise > select connection > Sync user profiles using SCIM
  2. Need to **TURN OFF** "Sync user profile attributes at each login"
  3. Select the Setup tab under "Sync user profiles using SCIM" to get the SCIM endpoint URL and 
     generate SCIM tokens needed for testing with Postman
* Supports CRUD, Search, except - PUT - replaces, PATCH - updates
* Recommendations - whitelist SCIM attributes you want to sync in a SCIM attribute map - what the fuck this is??
* What can you sync
  * users
  * roles
  * organisations
  * account linking
  * groups - not supported - use roles instead
